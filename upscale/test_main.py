...
class UpscaleServiceTests(unittest.TestCase):
    ...
    def test_finalize_output_creates_square_image(self):
        source = np.zeros((320, 520, 3), dtype=np.uint8)

        result = main.finalize_output(source)

        self.assertEqual(result.shape[0], 1080)
        self.assertEqual(result.shape[1], 1080)
    ...
    def test_endpoint_returns_square_jpeg(self):
        source = build_test_jpeg()
        upscale_result = np.zeros((540, 720, 3), dtype=np.uint8)
        square_result = np.zeros((1080, 1080, 3), dtype=np.uint8)

        with patch.object(main, "upscale_until_min_side", return_value=upscale_result), \
             patch.object(main, "finalize_output", return_value=square_result):
