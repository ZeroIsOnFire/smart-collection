class DetectedItemsController < ApplicationController
  ...
  before_action :set_detected_item, only: [:reject, :undo, :update_selection, :destroy]
  ...
  def create
    ...
    if @detected_item.save
      ...
      respond_to do |format|
        ...
        format.json { render json: @detected_item.errors, status: :unprocessable_entity }
      end
    end
  end
  ...
  def destroy
    ...
    respond_to do |format|
      ...
      format.html { redirect_back fallback_location: root_path, notice: "Item removido." }
    end
  end
