...
class GoogleVisionService
  TARGET_LABELS = %w[Toy Car Vehicle Model\ car].freeze
  ...
  def self.analyze(photo_path)
    ...
    begin
      image_annotator = Google::Cloud::Vision.image_annotator do |config|
        config.credentials = ENV['GOOGLE_CLOUD_CREDENTIALS_PATH']
      end
    ...
    response.responses.each do |res|
      res.localized_object_annotations.each do |obj|
        ...
        if TARGET_LABELS.any? { |label| obj.name.to_s.downcase.include?(label.downcase) }
          detected_items << {
            label: obj.name,
            score: obj.score,
  ...
  def self.simulation_mode?
    ENV['VISION_SIMULATION_MODE'] == 'true'
  end
  ...
  def self.credentials_configured?
    ENV['GOOGLE_CLOUD_CREDENTIALS_PATH'].present? && File.exist?(ENV['GOOGLE_CLOUD_CREDENTIALS_PATH'])
  end
end
